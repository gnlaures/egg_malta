<section class="s-defaultSection s-certificatesAndNumbers">
    <div class="s-defaultSection__head">
        <div class="u-container">
            <h2 class="u-ta-c">Lorem ipsum dolor consectetur.</h2>
        </div>
    </div>
    <div class="s-defaultSection__content">
        <div class="s-certificatesAndNumbers__swiper">
            <div class="u-container">
                <div class="c-swiperCertificates">
                    <div class="swiper-wrapper">
                        <div class="swiper-slide"><?php include('parts/components/c-cardIcon.php');?></div>
                        <div class="swiper-slide"><?php include('parts/components/c-cardIcon.php');?></div>
                        <div class="swiper-slide"><?php include('parts/components/c-cardIcon.php');?></div>
                        <div class="swiper-slide"><?php include('parts/components/c-cardIcon.php');?></div>
                    </div>
                </div>
            </div>
        </div>
        <div class="s-certificatesAndNumbers__captions">
            <div class="u-container">
                <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Quisque sed sagittis tortor. Ut tempor viverra purus, vel sodales enim feugiat quis. Vivamus vitae bibendum diam. Quisque varius et urna non ornare.</p>
                <p>Vestibulum sem sapien, tempus ut placerat a, ornare quis ligula. Sed condimentum mi ac vulputate cursus. Sed metus dui, aliquet ac viverra eget, posue.</p>
            </div>
        </div>
        <div class="s-certificatesAndNumbers__numbers">
            <div class="u-container">
                <div class="c-swiperCompanyNumbers">
                    <div class="swiper-wrapper">
                        <div class="swiper-slide"><?php include('parts/components/c-cardNumber.php');?></div>
                        <div class="swiper-slide"><?php include('parts/components/c-cardNumber.php');?></div>
                        <div class="swiper-slide"><?php include('parts/components/c-cardNumber.php');?></div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
